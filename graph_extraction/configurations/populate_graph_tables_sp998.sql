...
\copy mcgraph FROM 'mcgraph_adjacency_sp998.csv' WITH CSV HEADER;
\copy recograph FROM 'recograph_adjacency_sp998.csv' WITH CSV HEADER;

CREATE INDEX ON mcgraph (eid);
CREATE INDEX ON recograph (eid);

CREATE TABLE graph_sp998 AS 
SELECT 
  m.eid, 
...
CREATE INDEX ON graph_sp998 (eid);
