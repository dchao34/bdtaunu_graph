...
\copy mcgraph FROM 'mcgraph_adjacency_sigmc.csv' WITH CSV HEADER;
\copy recograph FROM 'recograph_adjacency_sigmc.csv' WITH CSV HEADER;

CREATE INDEX ON mcgraph (eid);
CREATE INDEX ON recograph (eid);

CREATE TABLE graph_sigmc AS 
SELECT 
  m.eid, 
...
CREATE INDEX ON graph_sigmc (eid);
